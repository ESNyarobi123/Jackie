<div class="space-y-6">
    <div class="flex items-start justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-(--color-smoke)">Create Plan</h1>
            <p class="text-sm text-[rgba(30,41,59,0.65)] mt-1">Set up a new subscription plan.</p>
        </div>
        <div class="flex gap-2 shrink-0">
            <flux:button variant="ghost" href="{{ route('admin.subscription-plans.index') }}" wire:navigate>Back</flux:button>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="lg:col-span-2 glass-card p-6 glass-soft-shadow space-y-5">
            <flux:field>
                ...
                <flux:input wire:model.live="name" placeholder="e.g. Monthly" />
                <flux:error name="name" />
            </flux:field>

            <flux:field>
                <flux:label>Slug</flux:label>
                <flux:input wire:model.blur="slug" placeholder="monthly" />
                <flux:error name="slug" />
            </flux:field>

            <flux:field>
                <flux:label>Description</flux:label>
                <flux:textarea wire:model.blur="description" rows="3" placeholder="What's included in this plan..." />
                <flux:error name="description" />
            </flux:field>

            <flux:field>
                <flux:label>Features (one per line)</flux:label>
                <flux:textarea wire:model.blur="features_input" rows="6" placeholder="Access to all courses&#10;Live class access&#10;Certificate on completion" />
        ...
        <div class="glass-card p-6 glass-soft-shadow space-y-5">
            ...
            @if($is_free_trial)
                <flux:field>
                    ...
                    <flux:input type="number" wire:model.blur="trial_days" min="1" max="365" placeholder="7" />
